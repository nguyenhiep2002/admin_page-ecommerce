...
@Component({
    selector: 'multi-file-input',
    templateUrl: './multi-file-input.component.html',
    styleUrls: ['./multi-file-input.component.scss'],
    providers: [TranslatePipe]
})
export class MultiFileInputComponent implements OnInit, OnChanges {
    ...
    async uploadFiles(): Promise<any[]> {
        try {
            ...
            this.files.forEach(f => {
                if (f.id) 
                    uploadedFiles.push(this.updateFileItem({ id: f.id, name: f.name }, f));
                else {
    ...
    async uploadMultiRequest(newFiles: any[]): Promise<any[]> {
        ...
        newFiles.forEach(f => {
            let formData: FormData = new FormData();
            formData.append('files', f.fileData, f.name);
            uploadPromise.push(this.uploadFileService.uploadFiles(formData).then(rs => {
                if (rs && rs.uploadedFiles && rs.uploadedFiles.length > 0) {
